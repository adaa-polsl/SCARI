...
public class RegressionActionSnC extends AbstractSeparateAndConquer {
    ...
    public RuleSetBase run(ExampleSet trainSet) {
        ...
        while (carryOn) {
            ...
            rule.getCoveredPositives().addAll(IntStream.rangeClosed(0, ses.size()).boxed().collect(Collectors.toList()));
            rule.getCoveredNegatives().addAll(IntStream.rangeClosed(0, ses.size()).boxed().collect(Collectors.toList()));
