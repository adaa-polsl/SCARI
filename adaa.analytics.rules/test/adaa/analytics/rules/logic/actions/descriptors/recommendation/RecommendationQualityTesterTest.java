 ...
 */
    ...
    public void testRecommendationQualityDataSet() throws OperatorException, OperatorCreationException {
        ...
        ExampleSet examples = ArffFileLoader.load(datasetFilePath, this.labelAttributeName);
        ...
        ActionMetaTable optimized = new OptimizedActionMetaTable(dist, this.qualityFunction, snc.getStableAttributes());
        ...
        for(int i = 0; i < set.size(); i++) {
            ...
            List<MetaAnalysisResult> resOptimized = optimized.analyze(example, (int)from, (int)to , pruningEnabled, true);
        ...
        List<Double> precRightRec = new ArrayList<Double>();
        List<Double> precRightSet = new ArrayList<Double>();
        ...
        for (List<MetaAnalysisResult> curr : resultOpt) {
            ...
            for (int i = 0; i < loopLimit; i++) {
                ...
                adhoc.append("Precision of right recommendation: " + other.calculate(rightCoverage) + System.lineSeparator());
                precRightRec.add(other.calculate(rightCoverage));
                adhoc.append("Precision of right best from set: " + bestPrec + System.lineSeparator());
                ...
                builder.add("Right subrule on train set precision=" + precision.calculate(rightCoverage) + " coverage=" + coverage.calculate(rightCoverage));
                ...
                if (dropOnlyFirstRecommendation && curr.size() > 1) {
                    ...
                    double nextMaxPrecision = skipped.stream()
                            .map(x -> x.getActionRule())
                            .map(x -> x.getRightRule().covers(set))
                            .map(x -> precision.calculate(x))
                            .max(Comparator.naturalOrder())
                            .orElse(Double.MIN_VALUE);
                    if (precision.calculate(rightCoverage) >= nextMaxPrecision) {
