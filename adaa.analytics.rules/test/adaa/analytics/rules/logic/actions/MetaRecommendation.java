...
public class MetaRecommendation extends Recommendation {
	...
	public void train(ExampleSet set) {
		...
		table = new OptimizedActionMetaTable(dist, new ClassificationMeasure(ClassificationMeasure.Correlation), engine.getStableAttributes());
	...
	public ActionRuleSet test(ExampleSet set) {
		...
		for(int i = 0; i < set.size(); i++) {
			...
			List<MetaAnalysisResult> res = table.analyze(example, fromClassId, toClassId, false, true);
			results.addAll(res);
		}
